...
getEntryFile();
//
let devName = 't-material'
Object.keys(weexEntry).forEach(function(key){
    console.log(key, weexEntry[key])
    if (key !== devName) {
        delete weexEntry[key];
    }
});
Object.keys(webEntry).forEach(function(key){
    if (key !== devName) {
        delete webEntry[key];
    }
});
